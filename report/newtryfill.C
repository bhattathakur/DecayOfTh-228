...
void newtryfill(){
  ...
  TH1F *h=new TH1F ("h","Histogram with energy Calibration",8192,-1.4,2900); 
  while(1)
    {
      in>>x>>y;
      if(!in.good())break;
      h->SetBinContent(x,y); //SetBinContent(bin,content)
      h->GetXaxis()->SetTitle("Enrgy/Channel (keV)");
 ...
 f5 ->SetParameters(762.376,2.848,24.0447,0,0);
 h5 ->Fit("f5","rem+","",760,765);//r-in given range,e->with better error estimation, m->improve fitting result,last two options are range of fit,
 //+->Add fitted fn to the list of functions
 gStyle->SetOptFit(1111);
 h5->Sumw2();
 h5 ->Draw();
 ...
 if(output.is_open())
   {
	 ...
	 <<(h->GetXaxis())->FindBin(f4->GetParameter(0))<<'\t'<<f4->GetParameter(0)<<'\n'
           //<<f5->GetParameter(0)<<'\t'<<(h->GetXaxis())->FindBin(f5->GetParameter(0))<<'\n'
